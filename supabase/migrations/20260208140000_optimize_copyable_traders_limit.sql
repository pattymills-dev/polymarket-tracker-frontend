-- Further optimize copyable refresh to avoid timeouts by limiting candidate traders.
-- We keep the same scoring logic, but only compute on a bounded set of active traders.

CREATE OR REPLACE FUNCTION refresh_copyable_traders(
  p_days INT DEFAULT 30,
  p_trade_limit INT DEFAULT 100,
  p_max_traders INT DEFAULT 5000
)
RETURNS void AS $$
DECLARE
...
BEGIN
  ...
  candidate_addrs AS (
    ...
    ORDER BY r.resolved_notional DESC NULLS LAST
    LIMIT p_max_traders
  ),
  per_trader_all AS (
    SELECT
