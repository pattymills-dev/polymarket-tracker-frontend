-- Optimize refresh_copyable_traders() to avoid full-table window sorts.
--
-- Problem:
-- The original implementation used ROW_NUMBER() over all trades, which can time out as the table grows.
--
-- Approach:
-- 1) Restrict candidate traders to those active in the last N days.
-- 2) Build the window as: (last N days trades) UNION (last K trades per active trader), deduped by tx_hash.
-- 3) Compute metrics and scores over that reduced dataset.
-- 4) Add indexes that match the access patterns (trader_address + timestamp DESC).

-- Indexes to support per-trader "latest trades" lookups and recent-window scans.
CREATE INDEX IF NOT EXISTS idx_trades_timestamp_desc
  ON trades (timestamp DESC);

CREATE INDEX IF NOT EXISTS idx_trades_trader_timestamp_desc
  ON trades (trader_address, timestamp DESC);

-- Partial index for BUY trades (we treat NULL side as BUY in our pipeline).
CREATE INDEX IF NOT EXISTS idx_trades_buy_trader_timestamp_desc
  ON trades (trader_address, timestamp DESC)
  WHERE side IS NULL OR side = 'BUY';
    ...
    SELECT
      ...
      COALESCE(
        COALESCE(r.wins, 0)::numeric /
          NULLIF(COALESCE(r.resolved_trades_count, 0)::numeric, 0),
        0
      )::numeric as win_rate,
      COALESCE(r.resolved_notional, 0)::numeric as resolved_notional,
      COALESCE(r.realized_pl, 0)::numeric as realized_pl,
      COALESCE(
        COALESCE(r.realized_pl, 0)::numeric /
          NULLIF(COALESCE(r.resolved_notional, 0)::numeric, 0),
        0
      )::numeric as realized_roi,
