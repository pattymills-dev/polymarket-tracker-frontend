-- Tune Copyable Traders ranking filters.
--
-- The original spec used avg(|price - 0.5|) >= 0.10 as a "payout potential" proxy.
-- In practice this can exclude perfectly reasonable, high-upside trades around 40-60c.
--
-- We already exclude extreme entry prices (<= 5c or >= 95c) and penalize traders who
-- skew toward extreme probabilities, so we remove the avg_abs_dist gate to ensure the
-- leaderboard populates with realistic copyable candidates.
...
BEGIN
  ...
  candidates AS (
    ...
    WHERE m.resolved_trades_count >= 10
