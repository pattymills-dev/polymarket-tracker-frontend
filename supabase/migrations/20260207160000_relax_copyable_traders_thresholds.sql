-- Relax Copyable Traders candidate thresholds and reduce runtime.
--
-- Why:
-- - In early datasets (or after excluding extreme entry prices), requiring >= 10 resolved trades can
--   produce an empty leaderboard.
-- - percentile_cont() (median) is expensive; compute it only for traders that pass cheap pre-filters.
--
-- Changes:
-- - Add an index on trades(tx_hash) to speed DISTINCT ON deduping.
-- - Pre-filter candidates by resolved_trades_count / resolved_notional before percentile_cont().
-- - Loosen resolved evidence threshold from 10 -> 5.
